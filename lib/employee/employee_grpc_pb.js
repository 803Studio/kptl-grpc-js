...
function serialize_global_ResponseHeader(arg) {
  if (!(arg instanceof global_headers_pb.ResponseHeader)) {
    throw new Error('Expected argument of type global.ResponseHeader');
  }
  return Buffer.from(arg.serializeBinary());
}

function deserialize_global_ResponseHeader(buffer_arg) {
  return global_headers_pb.ResponseHeader.deserializeBinary(new Uint8Array(buffer_arg));
}
...
var EmployeeService = exports.EmployeeService = {
  ...
  completeSelfInformation: {
    ...
    responseType: global_headers_pb.ResponseHeader,
    requestSerialize: serialize_employee_SelfInformation,
    requestDeserialize: deserialize_employee_SelfInformation,
    responseSerialize: serialize_global_ResponseHeader,
    responseDeserialize: deserialize_global_ResponseHeader,
  },
